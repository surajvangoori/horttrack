...
import { Plus, User, MapPin, Database, Download, Pencil, Trash2, Users, UserPlus, ChevronDown, ChevronUp, Mail, Lock, X } from 'lucide-react';
...
export default function AdminPanel({ user, profile }) {
  ...
  // Capture current browser location to automatically populate the form fields
  ...
  return (
    <div className="admin-grid">
      {/* LEFT COLUMN: Manage Client Sites */}
      ...
      {/* MIDDLE COLUMN: Employee Management */}
      <div className="card" style={{ padding: '1.5rem', alignSelf: 'start' }}>
        ...
        <div className="log-list" style={{ maxHeight: '500px' }}>
          {employees.length === 0 ? (
                  ...
                  {isExpanded && (
                    <div style={{ padding: '0 0.75rem 1rem', background: 'rgba(255,255,255,0.02)' }}>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
                        <button
                          className="btn btn-primary"
                          style={{ width: 'auto', padding: '0.3rem 0.85rem', fontSize: '0.8rem' }}
                        ...
                        <button
                          className="btn btn-secondary"
                          style={{ width: 'auto', padding: '0.3rem 0.85rem', fontSize: '0.8rem', color: 'var(--danger)', borderColor: 'rgba(239,68,68,0.3)' }}
                          ...
                          <>
                              ...
                              {assignedSites.length === 0 ? (
                                ...
                                assignedSites.map(site => (
                                  <span key={site.id} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', background: 'rgba(16,185,129,0.12)', color: 'var(--primary)', fontSize: '0.75rem', padding: '0.2rem 0.6rem', borderRadius: '999px', border: '1px solid rgba(16,185,129,0.25)' }}>
                                    {site.name}
                                    <button
                                      onClick={() => handleRemoveSite(emp.id, site.id)}
                            ...
                            {unassignedClients.length > 0 && (
                              <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                                ...
                                <button
                                  className="btn btn-secondary"
                                  style={{ width: 'auto', padding: '0.3rem 0.7rem', fontSize: '0.8rem', flexShrink: 0 }}
                                  disabled={!addSiteId || siteLoading}
                                  onClick={() => handleAssignSite(emp.id)}
                                >
                                  <Plus size={13} /> Assign
                                </button>
                              </div>
      ...
      {/* RIGHT COLUMN: Active / Historic shifts reports */}
      <div className="card" style={{ padding: '1.5rem' }}>
                  ...
                  <tr key={log.id} style={{ background: selectedSessionId === log.id ? 'rgba(255,255,255,0.05)' : 'none' }}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => handleViewSessionPings(log)}
                      >
                        Inspect Pings
                      </button>
                    </td>
        ...
        {/* Selected Session Background Pings Walkthrough */}
        {selectedSessionId && (
          <div style={{ marginTop: '2rem', padding: '1rem', border: '1px solid var(--card-border)', borderRadius: '8px', background: 'rgba(255,255,255,0.01)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div>
                <h4 style={{ fontSize: '1rem', fontWeight: '700', color: '#fff' }}>
                  Device Pings Audit Log
                </h4>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                  Showing GPS telemetry captured from worker device during shift.
                </p>
              </div>
              <button
                className="btn btn-secondary"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem' }}>
              ...
              {/* Spatial Map Simulator UI (Leaflet placeholder styled like custom radar layout) */}
                ...
                <div style={{ zIndex: 1 }}>
                  ...
                  <p style={{ fontSize: '0.75rem' }}>
                    {selectedSessionLogs.length > 0 
                      ...
                      : 'Waiting for coordinates map mapping...'}
                  </p>
                </div>
                {selectedSessionLogs.length > 0 && (
